...
def render_search_results(data: dict[str, Any]) -> None:
    ...
    # Print plain-text URLs for terminal click-to-open
    console.print()
    for i, item in enumerate(items, 1):
        url = _build_note_url(item["note_id"], item.get("xsec_token", ""), source="pc_search", route="search_result")
        console.print(f"  [dim]{i:2d}.[/dim] {url}")
