...
def _extract_from_firefox_sqlite() -> dict[str, str] | None:
    """Extract XHS cookies directly from Firefox SQLite databases (including Snap)."""
    import shutil
    import tempfile
    ...
    candidates += list(Path.home().glob("snap/firefox/common/.mozilla/firefox/*/cookies.sqlite"))
    ...
    for db_path in candidates:
        try:
            ...
            if cookies.get("a1"):
                logger.debug("Loaded XHS cookies from Firefox SQLite: %s", db_path)
                return cookies
        except Exception as exc:
            logger.debug("Failed to read Firefox cookies from %s: %s", db_path, exc)
...
def extract_browser_cookies(source: str = "auto") -> tuple[str, dict[str, str]] | None:
    ...
    # Try direct Firefox SQLite first (handles Snap installs)
    cookies = _extract_from_firefox_sqlite()
    if cookies:
